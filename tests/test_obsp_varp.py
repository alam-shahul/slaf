...
def test_chunked_h5ad_conversion_preserves_obsp(tmp_path, anndata_with_obsp_varp):
    """Chunked single-file conversion preserves pairwise observation matrices."""
    h5ad_path = tmp_path / "input.h5ad"
    slaf_path = tmp_path / "dataset.slaf"
    anndata_with_obsp_varp.write_h5ad(h5ad_path)
    ...
    converter.convert(str(h5ad_path), str(slaf_path))

    slaf = SLAFArray(slaf_path, load_metadata=False)
    ...
    np.testing.assert_array_almost_equal(
        ...
        anndata_with_obsp_varp.obsp["connectivities"],
    )
    np.testing.assert_array_almost_equal(
        lazy.obsp["distances"].toarray(),
        anndata_with_obsp_varp.obsp["distances"],
    )
