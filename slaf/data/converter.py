...
class SLAFConverter:
    ...
    def _convert_chunked(self, h5ad_path: str, output_path: str):
        ...
        with create_chunked_reader(
            ...
            obsp_keys: list[str] = []
            if (
                hasattr(reader, "adata")
                and reader.adata is not None
                and hasattr(reader.adata, "obsp")
                and reader.adata.obsp
            ):
                logger.info(
                    f"Converting {len(reader.adata.obsp)} obsp matrices in chunked mode..."
                )
                obsp_keys = self._convert_obsp(
                    reader.adata.obsp,
                    output_path,
